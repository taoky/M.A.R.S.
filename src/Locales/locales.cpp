...
bool load(std::string const & fileName)
{
                ...
                for (int i = 0; i < tmp.getSize(); ++i)
                {
                    if (tmp[i] == '{')
                    {
                        ...
                        if (macro == "PLAYER1_KEY_UP")
                            tmp.insert(
                                i, generateName::key(settings::C_playerIup));
                        else if (macro == "PLAYER2_KEY_UP")
                            tmp.insert(
                                i, generateName::key(settings::C_playerIIup));
                        else if (macro == "PLAYER1_KEY_RIGHT")
                            tmp.insert(
                                i, generateName::key(settings::C_playerIright));
                        else if (macro == "PLAYER2_KEY_RIGHT")
                            tmp.insert(i, generateName::key(
                                              settings::C_playerIIright));
                        else if (macro == "PLAYER1_KEY_LEFT")
                            tmp.insert(
                                i, generateName::key(settings::C_playerIleft));
                        else if (macro == "PLAYER2_KEY_LEFT")
                            tmp.insert(
                                i, generateName::key(settings::C_playerIIleft));
                        else if (macro == "PLAYER1_KEY_FIRE")
                            tmp.insert(
                                i, generateName::key(settings::C_playerIfire));
                        else if (macro == "PLAYER2_KEY_FIRE")
                            tmp.insert(
                                i, generateName::key(settings::C_playerIIfire));
                        else if (macro == "PLAYER1_KEY_SPECIAL")
                            tmp.insert(i, generateName::key(
                                              settings::C_playerISpecialKey));
                        else if (macro == "PLAYER2_KEY_SPECIAL")
                            tmp.insert(i, generateName::key(
                                              settings::C_playerIISpecialKey));
                            ...
                            std::cout
                                << "Error parsing " << fileName << ": At ID "
                                << id << " is an unknown macro "
                                << macro.toAnsiString() << "!" << std::endl;
                    }
                }
    ...
    {
        std::cout << "Failed to open locale " << fileName
                  << "! Interface will be messed up with errors...\n";
        return false;
    }
}
} // namespace
    ...
    if (file::load(settings::C_dataPath + "locales/locales.conf", lines))
    {
             ...
             it != lines.end(); ++it)
        {
            ...
            else
            {
                ...
                {
                    if (arg == "RTL")
                        newLocale.LTR_ = false;
                    else
                        newLocale.LTR_ = true;
                }
                else
                    std::cout << "Unrecognized flag \"" << flag << "\" in "
                              << settings::C_dataPath + "locales/locales.conf !"
                              << std::endl;
            }
        }
        ...
        {
            if (!load(settings::C_dataPath + "locales/" +
                      locales_[settings::C_languageID].fileName_))
            {
                std::cout
                    << "Failed to load " << settings::C_dataPath << "locales/"
                    << locales_[settings::C_languageID].fileName_.toAnsiString()
                    << "! Falling back to English..." << std::endl;
                settings::C_languageID = 0;
            }
        }
        else
        {
            std::cout << "Specified language in mars.conf doesn't match any in "
                         "locales.conf! Falling back to English..."
                      << std::endl;
            settings::C_languageID = 0;
        }
