...
{
    if (allTeams_.size() > 0)
        for (std::vector<Team *>::iterator it = allTeams_.begin();
             it != allTeams_.end(); ++it)
            (*it)->setHome(home);
    else
...
Team const * getTeamL() { return allTeams_[0]; }

Team const * getTeamR() { return allTeams_[1]; }

std::vector<Team *> const & getAllTeams() { return allTeams_; }
