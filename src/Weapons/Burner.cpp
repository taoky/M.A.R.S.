...
void Burner::draw(float alpha) const
{
    ...
    glVertex2f(0, parent_->radius() * 0.3f);
    glTexCoord2f(posX * 0.125f, (posY + 1) * 0.03125f);
    glVertex2f(0, -1.f * parent_->radius() * 0.3f);
    glTexCoord2f((posX + 1) * 0.125f, (posY + 1) * 0.03125f);
    glVertex2f(parent_->radius() * 3.f, -1.f * parent_->radius() * 0.3f);
    glTexCoord2f((posX + 1) * 0.125f, posY * 0.03125f);
    glVertex2f(parent_->radius() * 3.f, parent_->radius() * 0.3f);
    glEnd();
}
...
void Burner::fire() const
{
    ...
    if (time - timer_ > 0.05f)
    {
        ...
        for (int i = 0; i < 20; ++i)
        {
            particles::spawn(particles::pAmmoBurner,
                             parent_->location() +
                                 faceDirection * parent_->radius() * 1.5f,
                             faceDirection, parent_->velocity(), Color3f(),
                             parent_->getOwner());
            particles::spawn(particles::pHeatBurner,
                             parent_->location() +
                                 faceDirection * parent_->radius() * 1.5f,
                             faceDirection, parent_->velocity());
        }
