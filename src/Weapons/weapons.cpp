...
auto create(WeaponType type, Ship * parent) -> Weapon *
{
    ...
    if (parent->getOwner()->type() == controllers::cPlayer1 &&
        type != wNoWeapon && type != wInsta)
        settings::C_playerIWeapon = type;
    else if (parent->getOwner()->type() == controllers::cPlayer2 &&
             type != wNoWeapon && type != wInsta)
    ...
    {
    case wAFK47:
        return new AFK47(parent);
    case wBurner:
        return new Burner(parent);
    case wFist:
        return new Fist(parent);
    case wFlubba:
        return new Flubba(parent);
    case wH2OMG:
        return new H2OMG(parent);
    case wROFLE:
        return new ROFLE(parent);
    case wRocketLauncher:
        return new RocketLauncher(parent);
    case wInsta:
        return new Insta(parent);
    case wShotgun:
        return new Shotgun(parent);
    default:
        return new NoWeapon(parent);
    }
}
