...
void AFK47::draw(float alpha) const
{
    ...
    glVertex2f(0, parent_->radius() * 0.95f);
    glTexCoord2f(posX * 0.125f, (posY + 1) * 0.03125f);
    glVertex2f(0, parent_->radius() * 0.45f);
    glTexCoord2f((posX + 1) * 0.125f, (posY + 1) * 0.03125f);
    glVertex2f(parent_->radius() * 1.5f, parent_->radius() * 0.45f);
    glTexCoord2f((posX + 1) * 0.125f, posY * 0.03125f);
    glVertex2f(parent_->radius() * 1.5f, parent_->radius() * 0.95f);

    glTexCoord2f(posX * 0.125f, posY * 0.03125f);
    glVertex2f(0, -1.f * parent_->radius() * 0.95f);
    glTexCoord2f(posX * 0.125f, (posY + 1) * 0.03125f);
    glVertex2f(0, -1.f * parent_->radius() * 0.45f);
    glTexCoord2f((posX + 1) * 0.125f, (posY + 1) * 0.03125f);
    glVertex2f(parent_->radius() * 1.5f, -1.f * parent_->radius() * 0.45f);
    glTexCoord2f((posX + 1) * 0.125f, posY * 0.03125f);
    glVertex2f(parent_->radius() * 1.5f, -1.f * parent_->radius() * 0.95f);
    glEnd();
}
...
void AFK47::fire() const
{
    ...
    if (time - timer_ > 0.1)
    {
        ...
        particles::spawn(
            particles::pAmmoAFK47,
            Vector2f(parent_->location().x_ +
                         (faceDirection.x_ * parent_->radius() * 0.9 -
                          faceDirection.y_ * parent_->radius() * 0.9),
                     parent_->location().y_ +
                         (faceDirection.x_ * parent_->radius() * 0.7 +
                          faceDirection.y_ * parent_->radius() * 0.7)),
