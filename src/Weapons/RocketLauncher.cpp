...
void RocketLauncher::draw(float alpha) const
{
    ...
    glVertex2f(0, parent_->radius() * 0.5f);
    glTexCoord2f(posX * 0.125f, (posY + 1) * 0.03125f);
    glVertex2f(0, -1.f * parent_->radius() * 0.5f);
    glTexCoord2f((posX + 1) * 0.125f, (posY + 1) * 0.03125f);
    glVertex2f(parent_->radius() * 2.f, -1.f * parent_->radius() * 0.5f);
    glTexCoord2f((posX + 1) * 0.125f, posY * 0.03125f);
    glVertex2f(parent_->radius() * 2.f, parent_->radius() * 0.5f);
    glEnd();
}
