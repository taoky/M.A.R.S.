...
void TabList::keyEvent(bool down, Key const & key)
{
    if (focusedTab_)
    {
        ...
        {
            if (down && ((key.navi_ == Key::nLeft &&
                          locales::getCurrentLocale().LTR_) ||
                         (key.navi_ == Key::nRight &&
                          !locales::getCurrentLocale().LTR_)))
            {
                int i(0);
                while (tabs_[i] != focusedTab_)
                    i = (i - 1 + tabs_.size()) % tabs_.size();
                i = (i - 1 + tabs_.size()) % tabs_.size();
                while (!tabs_[i]->isTabable())
                    i = (i - 1 + tabs_.size()) % tabs_.size();
    ...
    if (focusedTab_)
    {
        if (focusedTab_->isFocused())
        {
            if (focusedTab_->tabPrevious())
            {
                menus::clearFocus();
                //
                // THIS ONE IS HACKY! the false below is a workaround to make
                // tabLists tabable from below... it's therefore impossible to
                // place tabLists inside of other Tablists...
                //
                setFocus(this, false);
            }
            return false;
...
void TabList::setFocus(UiElement * toBeFocused, bool isPrevious)
{
    ...
    else if (toBeFocused == this && isPrevious && focusedTab_ &&
             !focusedTab_->isFocused())
...
void TabList::addTab(Tab * toBeAdded)
{
    ...
    toBeAdded->label_ =
        new Label(toBeAdded->name_, TEXT_ALIGN_CENTER,
                  Vector2f(lastTabEnd_ + toBeAdded->width_ / 2, -6), 12.f,
                  Color3f(0.65f, 0.65f, 0.65f), false);
