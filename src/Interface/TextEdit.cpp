...
void TextEdit::mouseMoved(Vector2f const & position)
{
    ...
    if (pressed_ && sf::Mouse::isButtonPressed(sf::Mouse::Left))
    {
        ...
        while (text::getCharacterPos(*value_, cursorPos_, 12.f,
                                     TEXT_ALIGN_CENTER) +
                       2 * mirror + getTopLeft().x_ +
                       (width() + labelWidth_ * mirror) / 2 <
                   window::getMousePosition().x_ - 3 &&
               cursorPos_ < value_->getSize())
            ++cursorPos_;
    }
}
...
void TextEdit::keyEvent(bool down, Key const & key)
{
    if (key.type_ == Key::kKeyBoard)
    {
        if (pressed_)
        {
            ...
            {
                // backspace
                if (key.code_.keyBoard_ == sf::Keyboard::BackSpace &&
                    cursorPos_ > 0)
                {
                ...
                // delete
                else if (key.code_.keyBoard_ == sf::Keyboard::Delete &&
                         cursorPos_ < value_->getSize())
                {
                    value_->erase(cursorPos_, 1);
                }
                // move cursor
                else if (key.code_.keyBoard_ == sf::Keyboard::Left &&
                         cursorPos_ > 0)
                {
                    --cursorPos_;
                    cursorTimer_ = 0;
                }
                else if (key.code_.keyBoard_ == sf::Keyboard::Right &&
                         cursorPos_ < value_->getSize())
                {
                    ++cursorPos_;
                    cursorTimer_ = 0;
                }
                else if (key.navi_ == Key::nAbort ||
                         key.code_.keyBoard_ == sf::Keyboard::Up ||
                         key.code_.keyBoard_ == sf::Keyboard::Down ||
                         key.navi_ == Key::nConfirm)
                {
                    if (*value_ == "")
                        *value_ = fallBack_;
                    menus::unFixKeyboard();
...
void TextEdit::textEntered(sf::Uint32 keyCode)
{
    if (pressed_)
    {
        if (type_ == TEXT_EDIT)
        {
            if (value_->getSize() < maxLength_ && keyCode != 8 &&
                keyCode != 13 && keyCode != 32 && keyCode != 127)
            {
...
void TextEdit::draw() const
{
    ...
    if (pressed_)
        ...
        text::drawScreenText(
            *value_, origin + Vector2f((width() + labelWidth_ * mirror) / 2, 1),
            12.f, TEXT_ALIGN_CENTER, color);

    // draw cursor
    if (pressed_ && cursorTimer_ < 30)
    {
        int pos = text::getCharacterPos(*value_, cursorPos_, 12.f,
                                        TEXT_ALIGN_CENTER) +
                  2;
        glColor4f(1, 0.8, 0.9, 0.5);
        glLineWidth(0.5f);
        glBegin(GL_LINES);
        glVertex2f(origin.x_ + pos + (width() + labelWidth_ * mirror) / 2,
                   origin.y_ + 3);
        glVertex2f(origin.x_ + pos + (width() + labelWidth_ * mirror) / 2,
                   origin.y_ + height_ - 3);
        glEnd();
