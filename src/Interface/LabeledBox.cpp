    ...
    : UiElement(topLeft, width, height), label_(NULL)
{
    ...
    if (text)
    {
        label_ = new Label(text, TEXT_ALIGN_LEFT, Vector2f(5, 5), 12.f,
...
void LabeledBox::draw() const
{
    ...
    // dark background
    if (isTopMost())
        glColor4f(1.f, 0.5, 0.8, hoveredFadeTime_ * 0.05f);
    else
        glColor4f(1.0f, 1.0f, 1.0f, 0.f);
    glVertex2f(origin.x_, origin.y_);
    if (isTopMost())
        glColor4f(1.f, 0.5, 0.8, 0.f);
    else
        glColor4f(1.0f, 1.0f, 1.0f, 0.f);
    ...
    // dark background
    if (isTopMost())
        glColor4f(1.f, 0.5, 0.8, hoveredFadeTime_ * 0.15f + 0.45f);
    else
