...
VerticalSlider::VerticalSlider(float * value, float minValue, float maxValue,
    ...
    : UiElement(topLeft, 20, height), value_(value), minValue_(minValue),
      maxValue_(maxValue)
{
}
...
void VerticalSlider::mouseMoved(Vector2f const & position)
{
    ...
    {
        *value_ =
            (position.y_ + (0.5f * (height_ - 10)) / (maxValue_ - minValue_) -
             getTopLeft().y_ - 5) *
                (maxValue_ - minValue_) / (height_ - 10) +
            minValue_;
        if (*value_ < minValue_)
            *value_ = minValue_;
        if (*value_ > maxValue_)
