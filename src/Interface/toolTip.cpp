    ...
    if (wholeText.getSize() > 0)
    {
        ...
        for (unsigned int i = 0; i < wholeText.getSize(); ++i)
        {
            ...
            else if (wholeText[i] != ' ')
            {
                ...
                if (text::getCharacterPos(tmp, tmp.getSize(), 12.f,
                                          TEXT_ALIGN_LEFT) > width_)
                {
                    if (lastSpace == 0)
                    {
...
void drawToolTip(float alpha)
{
    ...
    glVertex2f(position_.x_ + width - cornerRadius * mirror,
               position_.y_ + height);
    ...
    for (int i = 0; i <= 360; i += 30)
    {
        ...
        {
            glVertex2f(position_.x_ + width,
                       position_.y_ + height + cornerRadius);
            glEnd();
            glBegin(GL_TRIANGLE_FAN);
            glVertex2f(position_.x_, position_.y_ + height);
        }
        ...
        {
            glVertex2f(position_.x_ + cornerRadius * mirror,
                       position_.y_ + height);
        }

        if (i < 90)
            cornerPosition = position_;
        else if (i < 180)
            cornerPosition = position_ + Vector2f(width, 0.f);
        else if (i < 270)
            cornerPosition = position_ + Vector2f(width, height);
        else
            cornerPosition = position_ + Vector2f(0.f, height);
