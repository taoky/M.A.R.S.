...
std::list<std::shared_ptr<AmmoFlubba>> AmmoFlubba::activeParticles_;

AmmoFlubba::AmmoFlubba(Vector2f const & location, Vector2f const & direction,
                       Vector2f const & velocity, Color3f const & color,
                       Player * damageSource)
    : Particle<AmmoFlubba>(spaceObjects::oAmmoFlubba, location, 8.f, 1.0f,
                           randomizer::random(12.f, 15.f))
{
...
void AmmoFlubba::update()
{
    ...
    if (lifeTime_ > totalLifeTime_)
    {
        ...
        for (int i = 0; i < rand; ++i)
            particles::spawn(particles::pMiniAmmoFlubba, location_, Vector2f(),
                             Vector2f(), Color3f(), damageSource_);
    }
}
