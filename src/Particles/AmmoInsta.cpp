...
std::list<std::shared_ptr<AmmoInsta>> AmmoInsta::activeParticles_;

AmmoInsta::AmmoInsta(Vector2f const & location, Vector2f const & direction,
...
AmmoInsta::~AmmoInsta() { trailEffects::detach(this); }
...
void AmmoInsta::draw() const
{
    ...
    const Vector2f topLeft(location_ + 2 * direction + normDirection),
    ...
    glVertex2f(topLeft.x_, topLeft.y_);
    glTexCoord2f(posX * 0.125f, (posY + 0.5f) * 0.125f);
    glVertex2f(bottomLeft.x_, bottomLeft.y_);
    glTexCoord2f((posX + 3.f) * 0.125f, (posY + 0.5f) * 0.125f);
    glVertex2f(bottomRight.x_, bottomRight.y_);
    glTexCoord2f((posX + 3.f) * 0.125f, posY * 0.125f);
                            ...
                            Vector2f const & velocity)
{
    ...
    if (strength > 50.f)
    {
        ...
        case spaceObjects::oShip:
            sound::playSound(sound::SniperShipCollide, location,
                             (strength - 50) / 3);
            break;
        ...
        case spaceObjects::oHome:
            sound::playSound(sound::SniperPlanetCollide, location,
                             (strength - 50) / 3);
            break;
                       ...
                       Team * team)
{
    ...
    for (int i = 0; i < 10 + settings::C_iDumb * 0.9f; ++i)
    {
        ...
        Vector2f to(from + velocity * resolution +
                    acceleration * resolution * resolution);
             ...
             ++it)
        {
            if ((*it)->attackable())
            {
                ...
                if (clockWise(velocity, shipRight - to) &&
                    !clockWise(velocity, shipLeft - to) &&
                    ...
                    !clockWise(orthoDir, to - shipRight))
                {
                    if ((*it)->getOwner()->team() != team)
                    {
                        /* glPointSize(50.f);
                         ...
                         glBegin(GL_POINTS);
                             glVertex2f((*it)->location().x_,
                         (*it)->location().y_); glEnd();*/
                        return 100;
                    }
                    else
                    {
        ...
        if (to.x_ < -100 || to.x_ > SPACE_X_RESOLUTION + 100 || to.y_ < -100 ||
