...
std::list<std::unique_ptr<CannonBall>> CannonBall::activeParticles_;

CannonBall::CannonBall(Vector2f const & location, Vector2f const & direction,
                       Vector2f const & velocity, Color3f const & color,
                       Player * damageSource)
    : Particle<CannonBall>(spaceObjects::oCannonBall, location, 25.f, 30.0f,
                           5.0f),
      timer1_(0.f), timer2_(0.f)
{
...
void CannonBall::update()
{
    ...
    else
    {
        ...
        for (int i = 0; i < 20; ++i)
        {
            particles::spawn(particles::pSmoke,
                             location_ + Vector2f::randDirLen() * 15.f,
                             velocity_);
    ...
    else
    {
        ...
        particles::spawn(particles::pFragmentFlame, location_,
                         Vector2f(20.f, 0), velocity_);
    }
