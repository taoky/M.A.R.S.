...
void BotController::draw()
{
    if (ship()->getLife() > 0)
    {
        ...
        const Vector2f shipDirection(Vector2f(std::cos(shipRotation),
        std::sin(shipRotation))); if (ship()->currentWeapon_->getType() ==
        weapons::wInsta) { slaveFire(AmmoInsta::hitsAny(ship()->location() +
        shipDirection*ship()->radius(), shipDirection, slave_->team()));
        }*/
        ...
        if (settings::C_drawBotJobs)
        {
            ...
            case Job::jCharge:
            {
                Vector2f direction(slave_->team()->home()->location() -
                                   shipLocation);
                decoObjects::drawArrow(shipLocation + direction * 0.05f,
                                       shipLocation + direction * 0.6f,
            ...
            {
                Vector2f direction(
                    *static_cast<Vector2f *>(currentJob_.object_) -
                    shipLocation);
                decoObjects::drawArrow(shipLocation + direction * 0.1f,
                                       shipLocation + direction * 0.9f,
                                       Color3f(0.6f, 0.f, 0.6f));
                break;
            }
            case Job::jKickToEnemy:
            {
                Vector2f direction(
                    static_cast<Ball *>(currentJob_.object_)->location() -
                    shipLocation);
                decoObjects::drawArrow(shipLocation + direction * 0.1f,
                                       shipLocation + direction * 0.9f,
                                       Color3f(0.8f, 0.4f, 0.6f));
                break;
            }
