...
{
    if (ship()->docked_ &&
        (weaponChangeTimer_ < 0.5f || specialChangeTimer_ < 0.5f) &&
        currentJob_.type_ != Job::jKickOutHome)
    {
        charge();
    }
    else
         ...
         it != jobMap.end(); ++it)
    {
        ...
        if (ship()->getLife() > 0 && ship()->frozen_ <= 0.f)
        {
            ...
            case Job::jCharge:
            {
                ...
                {
                    float life =
                        100.f - std::pow(ship()->getLife(), 5) * 0.00000001f;
                    float fuel =
                        100.f - std::pow(ship()->getFuel(), 5) * 0.00000001f;
            ...
            case Job::jHeal:
            {
                    ...
                    ship()->fragStars_ > 0)
                {
                    ...
                    if (target != ship())
                    {
                        ...
                        if (dist < 0)
                            dist = 0;
                        need = dist + (10 * ship()->fragStars_) +
                               didThisJobLAstTimeToo;
                    }
                }
                break;
            }
            ...
            case Job::jKickToEnemy:
            {
                ...
                float dir =
                    spaceObjects::isOnLine(ship()->location(),
                                           ballLocation - ship()->location(),
                                           targetPlanetLocation, 25.f)
                        ? 15
                        : 0;
            ...
            case Job::jKickOutHome:
            {
                ...
                // false);
                float dir = spaceObjects::isOnLine(
                                ballLocation, ship()->location() - ballLocation,
                                slave_->team()->home()->location(), 45.f)
                                ? 50
                                : 0;
            ...
            case Job::jGetControl:
            {
                ...
                {
                    float dist =
                        ((control->location() - ship()->location()).length());
                    dist *= 0.1f;
                    dist = 50 - dist;
                    if (dist < 0)
                        dist = 0;
