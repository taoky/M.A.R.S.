                           ...
                           bool avoidBall, float minDistance, bool goingToLand)
{
    ...
    const Vector2f shipDirection =
        Vector2f(std::cos(shipRotation), std::sin(shipRotation));

    if (nextPathPoint_ == location)
        aimDirection_ =
            nextPathPoint_ - shipLocation -
            shipVelocity * stopFactor * shipVelocity.lengthSquare() * 0.00003f;
    else
        aimDirection_ =
            nextPathPoint_ - shipLocation -
            shipVelocity * 0.8f * shipVelocity.lengthSquare() * 0.00003f;

    aimDirection_ = aimDirection_.normalize();

    turnTo(aimDirection_ + shipLocation);

    Vector2f velocityInAimDirection =
        aimDirection_ * (shipVelocity * aimDirection_);
    float distance = (nextPathPoint_ - shipLocation).lengthSquare();

    int accelerate(0);
    if (velocityInAimDirection.lengthSquare() < 15000.f + strength_ * 75000.f ||
        shipVelocity * aimDirection_ < 0.f)
        if (distance > 2500 || (shipVelocity * aimDirection_ < 0.f &&
                                (distance > 250.f || !goingToLand)))
            if (spaceObjects::isOnLine(shipLocation, shipDirection,
                                       shipLocation + aimDirection_ * 50.f,
                                       10.f))
                accelerate = 100;

    slaveUp(accelerate);

    return ((location - shipLocation).lengthSquare() <
...
bool BotController::turnTo(Vector2f const & location)
{
    ...
    float angle = aimDirection_.y_ * std::cos(shipRotation) -
                  aimDirection_.x_ * std::sin(shipRotation);
    if (angle > 0)
        slaveRight(100);
    else
        slaveLeft(100);
...
Vector2f BotController::calcPath(Vector2f const & endPoint, bool avoidBall)
{
    ...
    if (++pathDepth < 5)
    {
        ...
        if (obstacle != NULL)
        {
            ...
            {
                pathDepth = 0;
                Vector2f surfacePoint(obstacle->location() -
                ...
                while (!fits && ++count < 6)
                {
                         ...
                         it != allShips.end(); ++it)
                    {
                                ...
                                225.f)
                        {
                            fits = false;
                            Vector2f rotated;
                            float phi = std::pow(-1.f, count) *
                                        ((count + 1) / 2) * M_PI / 6.f;
                            if (toEndPoint.x_ != 0.f)
                                rotated = Vector2f(
                                    std::cos(std::atan2(toEndPoint.y_,
                                                        toEndPoint.x_) +
                                             phi),
                                    std::sin(std::atan2(toEndPoint.y_,
                                                        toEndPoint.x_) +
                                             phi));
                            else
                                rotated =
                                    Vector2f(-std::sin(phi), std::cos(phi));
                            surfacePoint =
                                (obstacle->location() -
                                 rotated * (obstacle->radius() + 20.f));
                            break;
                        }
                    }
                    if (surfacePoint.x_ < 0.f ||
                        surfacePoint.x_ > SPACE_X_RESOLUTION)
                        fits = false;
                }
                if (surfacePoint.x_ > 0.f &&
                    surfacePoint.x_ < SPACE_X_RESOLUTION)
            ...
            if (obstacleToStart * normalToPath == 0.f)
                newEndPoint = obstacle->location() -
                              normalToPath * (obstacle->radius() + 60.f);
            else
            {
                normalToPath = (normalToPath * (obstacleToStart * normalToPath))
                                   .normalize();
                newEndPoint = obstacle->location() +
                              normalToPath * (obstacle->radius() + 60.f);
            }
            // check whether new endPoint is in an obstacle again... (happens,
            // whenever two obstacles are close to each other)
            SpaceObject const * newObstacle = spaceObjects::getObstacle(
            ...
            if (newObstacle != NULL && obstacle != newObstacle)
            {
                ...
                Vector2f obst1 =
                    obstacle->location() + obst1obst2 * obstacle->radius();
                Vector2f obst2 = newObstacle->location() -
                                 obst1obst2 * newObstacle->radius();
...
void BotController::shootPoint(Vector2f const & location, bool avoidTeamMembers)
{
    ...
    {
        slaveFire(AmmoInsta::hitsAny(ship()->location() +
                                         shipDirection * ship()->radius(),
                                     shipDirection, slave_->team()));
    }
    else
    {
        ...
        {
            if (spaceObjects::isOnLine(ship()->location(), shipDirection,
                                       location, maxAngle))
            {
                int doShoot(100);
                if (avoidTeamMembers)
