...
void KeyController::update(Key const & key) const
{
    ...
    {
        if (key == settings::C_playerIup)
            slaveUp();
        else if (key == settings::C_playerIleft)
            slaveLeft();
        else if (key == settings::C_playerIright)
            slaveRight();
        else if (key == settings::C_playerIfire)
            slaveFire();
        else if (key == settings::C_playerISpecialKey)
            slaveSpecial();
    }

    else if (type_ == controllers::cPlayer2 && key.strength_ == 100)
    {
        if (key == settings::C_playerIIup)
            slaveUp();
        else if (key == settings::C_playerIIleft)
            slaveLeft();
        else if (key == settings::C_playerIIright)
            slaveRight();
        else if (key == settings::C_playerIIfire)
            slaveFire();
        else if (key == settings::C_playerIISpecialKey)
            slaveSpecial();
    }
}
