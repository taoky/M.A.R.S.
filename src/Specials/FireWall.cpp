...
void FireWall::draw(float alpha) const
{
    ...
    if (timer_ > 0.f)
    {
        ...
        else
        {
            ...
            {
                burnTimer_ = timer_;
                Vector2f dir(std::cos(timer::totalTime() * 3),
                             std::sin(timer::totalTime() * 3));
                for (int i = 0; i < 20; ++i)
                {
                    particles::spawn(
                        particles::pAmmoBurner,
                        parent_->location_ + Vector2f(dir.x_, dir.y_) *
                                                 parent_->radius() * 1.25f,
                        Vector2f(dir.x_, dir.y_), parent_->velocity(),
                        Color3f(), parent_->getOwner());
                    particles::spawn(
                        particles::pAmmoBurner,
                        parent_->location_ + Vector2f(-dir.x_, -dir.y_) *
                                                 parent_->radius() * 1.25f,
                    ...
                    particles::spawn(
                        particles::pAmmoBurner,
                        parent_->location_ + Vector2f(dir.y_, -dir.x_) *
                                                 parent_->radius() * 1.25f,
                        Vector2f(dir.y_, -dir.x_), parent_->velocity(),
                        Color3f(), parent_->getOwner());
                    particles::spawn(
                        particles::pAmmoBurner,
                        parent_->location_ + Vector2f(-dir.y_, dir.x_) *
