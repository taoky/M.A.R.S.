...
Home::Home(Vector2f const & location, int life, float radius, float mass,
    ...
    : SpaceObject(spaceObjects::oHome, location, radius, mass),
      color_(color.brightened()), life_(life), visible_(true)
{
...
void Home::draw() const
{
    if (visible_)
    {
        ...
        glBegin(GL_QUADS);
        glTexCoord2i(0, 0);
        glVertex2f(location_.x_ - drawRadius, location_.y_ - drawRadius);
        glTexCoord2i(0, 1);
        glVertex2f(location_.x_ - drawRadius, location_.y_ + drawRadius);
        glTexCoord2i(1, 1);
        glVertex2f(location_.x_ + drawRadius, location_.y_ + drawRadius);
...
void Home::drawLife() const
{
    ...
    if (visible_)
    {
        ...
        text::drawSpaceText(sf::String(sstr.str()),
                            location_ + Vector2f(xOffset, -30 + lifeSize),
                            lifeSize, TEXT_ALIGN_CENTER,
                            Color3f(0.9, 0.9, 0.9));
    }
}
...
void Home::createShips(std::vector<Player *> & inhabitants) const
{
    ...
    if (location_.x_ < radius_)
    {
             ...
             it != inhabitants.end(); ++it)
        {
            ...
            Vector2f location = Vector2f(std::cos(angle), std::sin(angle)) *
                                    (radius_ + SHIP_RADIUS) +
                                location_;
                       ...
                       Vector2f const & direction, Vector2f const & velocity)
{
    ...
    case spaceObjects::oAmmoInsta:
        particles::spawnMultiple(20, particles::pMud, location, direction,
                                 velocity, color_);
        break;

    case spaceObjects::oAmmoShotgun:
        particles::spawnMultiple(2, particles::pMud, location, direction,
                                 velocity, color_);
        break;
    ...
    case spaceObjects::oShip:
        if (strength > 50)
            particles::spawnMultiple(10, particles::pMud, location, direction,
                                     velocity, color_);
        break;
    ...
    case spaceObjects::oBall:
    {
        ...
        life_ -= amount;
        teams::getTeamL()->home() == this ? teams::getTeamR()->addStars()
                                          : teams::getTeamL()->addStars();
