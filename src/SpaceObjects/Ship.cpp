...
Ship::Ship(Vector2f const & location, float rotation, Player * owner)
    : MobileSpaceObject(spaceObjects::oShip, location, SHIP_RADIUS, 10.f),
      owner_(owner), rotation_(rotation), rotateSpeed_(1.f), up_(0), left_(0),
...
void Ship::update()
{
    ...
    if (damageCheckTimer_ > 0.f)
    {
        ...
        if (damageCheckTimer_ <= 0.f)
        {
            ...
            {
                particles::spawn(
                    particles::pNumber, location_ + Vector2f(0.f, -20.f),
                    Vector2f(damage, 20.f + std::abs(damage) * 0.02f),
    ...
    if (visible_)
    {
        ...
        if ((games::elapsedTime() > 4.f) || (games::type() == games::gTutorial))
        {
            ...
            if (frozen_ <= 0)
            {
                ...
                {
                    if (right_ > 15)
                        fmod(rotation_ -= rotateSpeed_ * time * 0.3f * right_,
                             360.f);
                    if (left_ > 15)
                        fmod(rotation_ += rotateSpeed_ * time * 0.3f * left_,
                             360.f);
                }
                else
                {
                ...
                if (up_ > 15 && getFuel() > 0.f)
                {
                    ...
                    particles::spawnTimed(
                        1.5f / settings::C_globalParticleCount * up_,
                        particles::pFuel, location_ - faceDirection * radius_,
                        faceDirection, velocity_);
                    particles::spawnTimed(
                        0.1f / settings::C_globalParticleCount * up_,
                        particles::pHeatJet,
                        location_ - faceDirection * radius_ * 1.5f,
                        faceDirection, velocity_);
                }
                else
                {
                ...
                bool closeToHome(
                    toHome.lengthSquare() <
                    std::pow(home->radius() + radius_ + 0.1f, 2.f));
                if (up_ < 10 && velocity_.lengthSquare() < 13000.f &&
                    closeToHome &&
                    ((faceDirection + toHome.normalize()).lengthSquare() <
                     0.26f))
                {
                    docked_ = true;
                    velocity_ = Vector2f();
                ...
                // s = s0 + v0*t + 0.5*a*t*t
                location_ +=
                    velocity_ * time + acceleration * 0.5f * time * time;
                // v = v0 + a*t
            ...
            else
            {
                ...
                if (frozen_ <= 0.f)
                {
                    frozen_ = 0.f;
                    mass_ = 10.f;
                    particles::spawnMultiple(2, particles::pCrushedIce,
                                             location_);
                }

                velocity_ = Vector2f();
                if (damageSource_ == players::getPlayerI() ||
                    damageSource_ == players::getPlayerII() ||
                    owner_ == players::getPlayerI() ||
                    owner_ == players::getPlayerII())
                {
...
void Ship::draw() const
{
    if (visible_)
    {
        ...
        float x, y,
            alpha(
                ghostTimer_ == 1.f
                    ? 0.2f * std::sin(timer::totalTime() * 8.f + 1.5f * M_PI) +
                          0.4f
                    : (ghostTimer_ > 0.f
                           ? ghostTimer_ *
                                     (0.2f * std::sin(timer::totalTime() * 8.f +
                                                      1.5f * M_PI) +
                                      0.4f) +
                                 1.f - ghostTimer_
                           : 1.f));
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(x, y + 0.125f);
        glVertex2f(-radius_, -radius_);
        glTexCoord2f(x + 0.125f, y + 0.125f);
        glVertex2f(-radius_, radius_);
        glTexCoord2f(x + 0.125f, y);
        glVertex2f(radius_, radius_);
        glTexCoord2f(x, y);
        glVertex2f(radius_, -radius_);
        glEnd();
    ...
    else if (respawnTimer_ > 6.f)
    {
        ...
        glVertex2f(-radius_ * 3.6f, -radius_ * 3.6f);
        glTexCoord2f(0.f, 0.875f);
        glVertex2f(-radius_ * 3.6f, radius_ * 3.6f);
        glTexCoord2f(0.125f, 0.875f);
        glVertex2f(radius_ * 3.6f, radius_ * 3.6f);
...
void Ship::drawWeapon() const
{
    if (visible_)
    {
        ...
        float alpha(
            ghostTimer_ == 1.f
                ? 0.2f * std::sin(timer::totalTime() * 8.f + 1.5f * M_PI) + 0.4f
                : (ghostTimer_ > 0.f
                       ? ghostTimer_ *
                                 (0.2f * std::sin(timer::totalTime() * 8.f +
                                                  1.5f * M_PI) +
                       ...
                       Vector2f const & direction, Vector2f const & velocity)
{
    ...
    case spaceObjects::oSun:
        ...
        if (strength > 50)
            sound::playSound(sound::BallPlanetCollide, location,
                             (strength - 50) / 3);
        break;
    ...
    case spaceObjects::oPlanet:
        ...
        if (strength > 50)
            sound::playSound(sound::ShipPlanetCollide, location,
                             (strength - 50) / 3);
        break;
    ...
    case spaceObjects::oBall:
        ...
        particles::spawnMultiple(2, particles::pSpark, location,
                                 direction * 100.f, velocity_, owner_->color());
        if (strength > 50)
            sound::playSound(sound::ShipPlanetCollide, location,
                             (strength - 50) / 3);
        unfreeze = 10.f;
    ...
    case spaceObjects::oAmmoAFK47:
        ...
        setDamageSource(with->damageSource());
        particles::spawnMultiple(
            2, particles::pSpark, location,
            dynamic_cast<MobileSpaceObject *>(with)->velocity() * 0.3f,
            velocity_, owner_->color());
        unfreeze = 0.1f;
        break;
    ...
    case spaceObjects::oAmmoBurner:
        ...
        if (randomizer::random(0.f, 100.f) / settings::C_globalParticleCount <
            0.01f)
...
void Ship::explode()
{
    ...
    particles::spawnMultiple(5, particles::pFragment, location_, location_,
                             location_, owner_->color());
    ...
    else if (damageSource_->team() == owner_->team())
    {
        ...
        (damageSource_->ship()->fragStars_ - 1 < 0)
