    ...
    if (visible_)
    {
        ...
        if (frozen_ <= 0.f)
        {
            ...
            else
            {
                ...
                // s = s0 + v0*t + 0.5*a*t*t
                location_ +=
                    velocity_ * time + acceleration * 0.5 * time * time;
                // v = v0 + a*t
            ...
            if (heatTimer_ > 0.f)
            {
                ...
                {
                    smokeTimer_ =
                        0.3f / (settings::C_globalParticleCount * heatTimer_);
                    for (int i = 0; i < 5; ++i)
                    {
                        particles::spawn(particles::pSmoke,
                                         location_ +
                                             Vector2f::randDirLen() * radius_,
                                         velocity_);
                    }
                }
            }
                       ...
                       Vector2f const & direction, Vector2f const & velocity)
{
    ...
    case spaceObjects::oSun:
        rotateSpeed_ =
            rotateSpeed_ * 0.5 +
            ((velocity_.x_ * direction.y_ - direction.x_ * velocity_.y_) *
             0.1) *
                0.5;
        heatTimer_ = 20;
        if (strength > 50)
    ...
    case spaceObjects::oShip:
        particles::spawnMultiple(1, particles::pSpark, location, direction,
                                 velocity_, Color3f(0.3f, 0.3f, 0.3f));
        if (strength > 50)
        {
            sound::playSound(sound::ShipBallCollide, location,
                             (strength - 50) / 3);
        }
        break;
...
void Ball::explode()
{
    ...
    particles::spawnMultiple(5, particles::pFragment, location_, location_,
                             location_, Color3f(0.3f, 0.3f, 0.3f));
