    ...
    if (with & STATICS)
    {
             ...
             it != staticObjects_.end(); ++it)
        {
                ...
                std::pow((*it)->radius_ + object->radius_, 2))
            {
                ...
                {
                    const Vector2f velocityBefore =
                        (object->velocity() * impactDirection) *
                        impactDirection;
    ...
    if (with & MOBILES)
    {
             ...
             it != mobileObjects_.end(); ++it)
        {
            ...
            if (*it != object)
            {
                ...
                {
                    const Vector2f velocitySourceNorm(
                        source->velocity().normalize());
                    const Vector2f velocitySourceOrtho(-velocitySourceNorm.y_,
                                                       velocitySourceNorm.x_);
                    const Vector2f centerDist(
                        velocitySourceOrtho *
                        ((source->location_ - target->location_) *
                         velocitySourceOrtho));

                    // if path of object crosses target object
                    if (centerDist.lengthSquare() < minDistSquared)
                    {
                        const Vector2f lastFrameLocation =
                            source->location_ -
                            source->velocity() * timer::frameTime() * 0.6f;
                        const Vector2f chordMidPoint =
                            target->location_ + centerDist;

                        // if path of object has intersected with target within
                        // the last frame
                        if (((source->location_ - target->location_)
                                 .lengthSquare() < minDistSquared) ||
                            ((lastFrameLocation - target->location_)
                                 .lengthSquare() < minDistSquared) ||
                            ((chordMidPoint - source->location_) *
                                 (chordMidPoint - lastFrameLocation) <
                             0.f))
                        {

                            if (source->velocity() *
                                    (target->location_ - lastFrameLocation) >
                                0)
                            {

                                const Vector2f moveOutDist(
                                    velocitySourceNorm *
                                    std::sqrt(minDistSquared -
                                              centerDist.lengthSquare()));
                                ...
                                const Vector2f impactLocation(
                                    ...
                                    0.5f);
                                const Vector2f impactDirection(
                                    (target->location_ - source->location_)
                                        .normalize());

                                // calculate velocity of objects in direction of
                                // impact before collision
                                const Vector2f velocitySourceBefore =
                                    impactDirection *
                                    (source->velocity() * impactDirection);
                                const Vector2f velocityTargetBefore =
                                    impactDirection *
                                    (target->velocity() * impactDirection);

                                if ((velocitySourceBefore -
                                     velocityTargetBefore)
                                        .lengthSquare() > 1000)
                                {
                                    // calculate velocity of objects in
                                    // direction of impact after collision
                                    const Vector2f velocitySourceAfter =
                                        (velocitySourceBefore * source->mass_ +
                                         velocityTargetBefore * target->mass_ -
                                         (velocitySourceBefore -
                                          velocityTargetBefore) *
                                             target->mass_) /
                                        (source->mass_ + target->mass_);
                                    const Vector2f velocityTargetAfter =
                                        (velocitySourceBefore * source->mass_ +
                                         velocityTargetBefore * target->mass_ -
                                         (velocityTargetBefore -
                                          velocitySourceBefore) *
                                    ...
                                    target->onCollision(source, impactLocation,
                                                        impactDirection,
                                                        velocitySourceBefore);
                                    source->onCollision(target, impactLocation,
                                                        impactDirection,
                                                        velocityTargetBefore);

                                    // add to orthongonal speed component of
                                    // initial velocity special case: Collision
                                    // with rofle bullets is not physically
                                    // correct, for improved gameplay
                                    if ((source->type() ==
                                         spaceObjects::oAmmoROFLE) ||
                                        (target->type() ==
                                         spaceObjects::oAmmoROFLE))
                                        target->velocity() +=
                                            (0.05f * source->velocity() *
                                                 source->mass_ +
                                             (velocityTargetAfter -
                                              velocityTargetBefore) *
                                                 0.6);
                ...
                else if ((source->location_ - target->location_)
                             .lengthSquare() < minDistSquared)
                {
                    // if objects intersect, but aren't moving
                    const Vector2f normDistance =
                        (target->location_ - source->location_).normalize();
                    const float moveOutDist =
                        ((target->radius_ + source->radius_) -
                         (source->location_ - target->location_).length()) *
                        0.5;
                    source->location_ =
                        target->location_ - normDistance * moveOutDist;
                    target->location_ =
                        source->location_ + normDistance * moveOutDist;
                }
            }
        }
    }
...
void addStaticObject(SpaceObject * object) { staticObjects_.push_back(object); }
