...
void draw()
{
    ...
    if (settings::C_StarsHigh)
        ...
        case 0:
            glBindTexture(GL_TEXTURE_2D,
                          texture::getTexture(texture::Stars1_large));
            break;
        case 1:
            glBindTexture(GL_TEXTURE_2D,
                          texture::getTexture(texture::Stars2_large));
            break;
        }
    else
        switch (tex_)
        ...
        case 0:
            glBindTexture(GL_TEXTURE_2D,
                          texture::getTexture(texture::Stars1_medium));
            break;
        case 1:
            glBindTexture(GL_TEXTURE_2D,
                          texture::getTexture(texture::Stars2_medium));
            break;
        }
    ...
    glVertex2f(0, 0);
    glTexCoord2f(left_, top_ + res * viewport.y_);
    glVertex2f(0, viewport.y_);
    glTexCoord2f(left_ + res * viewport.x_, top_ + res * viewport.y_);
    glVertex2f(viewport.x_, viewport.y_);
    glTexCoord2f(left_ + res * viewport.x_, top_);
    glVertex2f(viewport.x_, 0);
    glEnd();
