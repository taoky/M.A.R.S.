...
namespace
{
void drawText(sf::String const & text, Vector2f const & location, float size,
              int align, Color3f const & color, float alpha, sf::Font * font)
{

    sf::Text drawString(text, font ? *font : *font::getFont(), size);
    drawString.setFillColor(sf::Color(color.r() * 255, color.g() * 255,
                                      color.b() * 255,
                                      alpha < 0 ? 0 : alpha * 255));
    ...
    if (loc.y_ < 0.f)
        loc.y_ = 0.f;
    if (loc.x_ + boundingBox.width > port.x_)
        loc.x_ = port.x_ - static_cast<int>(boundingBox.width);
    if (loc.y_ + boundingBox.height > port.y_)
        loc.y_ = port.y_ - static_cast<int>(boundingBox.height);
...
void drawSpaceText(sf::String const & text, Vector2f const & location,
                   float size, int align, Color3f const & color, float alpha,
                   sf::Font * font)
{
...
void drawMobileSpaceText(sf::String const & text, Vector2f const & location,
                         ...
                         float alpha, sf::Font * font)
{

    drawText(text, window::coordToPixel(location), size, align, color, alpha,
             font);
}
                    ...
                    sf::Font * font)
{
    ...
    drawText(
        text,
        Vector2f(static_cast<int>(location.x_), static_cast<int>(location.y_)),
        size, align, color, alpha, font);
}

void drawFooText()
{
    // drawScreenText(sf::String("."), Vector2f(0.f, 0.f), 1.f, TEXT_ALIGN_LEFT,
    // Color3f(0.f, 0.f, 0.f));
}
