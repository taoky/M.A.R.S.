...
namespace
{
// for Music there is only one channel... who wants to have multiple music files
// played at once?
...
void update()
{
    if (settings::C_musicVolume > 0)
    {
        ...
        if (fadeOutTimer_ > 0.f)
        {
            ...
            if (fadeOutTimer_ < 0.f)
                fadeOutTimer_ = 0.f;
            musicChannel_.setVolume(settings::C_musicVolume * fadeOutTimer_ *
                                    2.5f);
        }
        ...
        else if (slowMoTime > 0.0f)
        {
            ...
            musicChannel_.setPitch(1.f);

        if (games::type() != games::gMenu &&
            games::type() != games::gTutorial &&
            window::isKeyDown(settings::C_statisticsKey))
            musicNotify::show(settings::C_dataPath + "/audio/music/" +
                              files_[playList_.back()]);
    }
...
void stop() { musicChannel_.stop(); }

void fadeOut() { fadeOutTimer_ = 0.5f; }
