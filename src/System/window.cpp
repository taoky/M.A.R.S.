...
const float ratio(static_cast<float>(SPACE_X_RESOLUTION) /
                  static_cast<float>(SPACE_Y_RESOLUTION));

void setViewPort()
{
    const int windowHeight(window_.getSize().y),
        windowWidth(window_.getSize().x);
    ...
    if (static_cast<float>(windowWidth) / windowHeight > ratio)
    {
        ...
        glViewport((windowWidth - viewPort_.x_) * 0.5f, 0, viewPort_.x_,
                   viewPort_.y_);
    }
    else
    {
        ...
        glViewport(0, (windowHeight - viewPort_.y_) * 0.5f, viewPort_.x_,
                   viewPort_.y_);
    }
...
void update()
{
    ...
    while (window_.pollEvent(event))
    {
        ...
        else if (event.type == sf::Event::MouseMoved)
        {
            if (menus::visible())
                menus::mouseMoved(
                    Vector2f(event.mouseMove.x -
                                 (window_.getSize().x - viewPort_.x_) / 2,
                             event.mouseMove.y -
                                 (window_.getSize().y - viewPort_.y_) / 2));
        }
        ...
        else if (event.type == sf::Event::JoystickButtonPressed)
        {
            ...
            {
                if (!menus::visible())
                    controllers::singleKeyEvent(
                        Key(event.joystickButton.joystickId,
                            event.joystickButton.button));
                menus::keyEvent(true, Key(event.joystickButton.joystickId,
                                          event.joystickButton.button));
                joyButtonTimer_ = timer::realTotalTime();
            }
        }
        else if (event.type == sf::Event::JoystickButtonReleased)
            menus::keyEvent(false, Key(event.joystickButton.joystickId,
                                       event.joystickButton.button));
        else if (event.type == sf::Event::JoystickMoved)
        {
            Key key(event.joystickMove.joystickId, event.joystickMove.axis,
                    event.joystickMove.position);
            if (key.strength_ >= 95 &&
                timer::realTotalTime() - joyButtonTimer_ > 0.1f)
            {
                if (!menus::visible())
...
int isKeyDown(Key const & key)
{
    ...
    case Key::kJoyAxis:
        ...
        std::pair<Key::AxisType, int> result(
            Key::convertFromSFML(tmp, strength));
...
void screenShot()
{
    ...
    // const int windowHeight(window_.GetHeight()),
    // windowWidth(window_.GetWidth()); if
    // (static_cast<float>(windowWidth)/windowHeight > ratio)
    //     shot.Copy(window_, sf::IntRect((windowWidth-viewPort_.x_)*0.5f, 0,
    //     viewPort_.x_, viewPort_.y_));
    // else
    //     shot.Copy(window_, sf::IntRect(0, (windowHeight-viewPort_.y_)*0.5f,
    //     viewPort_.x_, viewPort_.y_));
    ...
    filename << "ScreenShot_" << timeinfo->tm_year << timeinfo->tm_mon
             ...
             << timeinfo->tm_sec << "." << settings::C_screenShotFormat;

#ifdef __linux__
    mkdir((settings::C_configPath + "screenshots/").c_str(),
          S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    if (shot.saveToFile(settings::C_configPath + "screenshots/" +
                        filename.str()))
    {
        std::cout << "Saved screenshot to " << settings::C_configPath
                  << "screenshots/" << filename.str() << "." << std::endl;
        hud::displayMessage(*locales::getLocale(locales::SavedScreenshot));
    }
    else
    {
        std::cout << "Failed saving screenshot to " << settings::C_configPath
                  << "screenshots/" << filename.str() << "." << std::endl;
    }
#endif
