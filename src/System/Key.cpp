...
Key::Key(unsigned int joyID, sf::Joystick::Axis joyAxis, int strength)
    : type_(kJoyAxis), navi_(nNone), strength_(0), joyID_(joyID)
{
    ...
    if ((joyAxis == 7 && strength == -100) ||
        (joyAxis == 1 && strength == -100) ||
        ...
        navi_ = nUp;
    else if ((joyAxis == 7 && strength == 100) ||
             (joyAxis == 1 && strength == 100) ||
#if defined __WIN32__
             (joyAxis == 3 && strength == 100)
#else
        ...
        navi_ = nDown;
    else if ((joyAxis == 6 && strength == -100) ||
             (joyAxis == 0 && strength == -100) ||
             (joyAxis == 4 && strength == -100))
        navi_ = nLeft;
    else if ((joyAxis == 6 && strength == 100) ||
             (joyAxis == 0 && strength == 100) ||
             (joyAxis == 4 && strength == 100))
...
Key::Key(unsigned int joyID, unsigned int joyButton)
    : type_(kJoyButton), navi_(nNone), strength_(100), joyID_(joyID)
{
                                                   ...
                                                   int strength)
{
    ...
    case sf::Joystick::X:
        if (strength < 0)
            result.first = aALleft;
        else
            result.first = aALright;
        break;
    case sf::Joystick::Y:
        if (strength < 0)
            result.first = aALup;
        else
            result.first = aALdown;
        break;
    ...
    case sf::Joystick::U:
        if (strength < 0)
            result.first = aARleft;
        else
            result.first = aARright;
        break;
    case sf::Joystick::V:
#if !defined __WIN32__
        if (strength < 0)
    ...
    case sf::Joystick::PovX:
        if (strength < 0)
            result.first = aPOVleft;
        else
            result.first = aPOVright;
        break;
    case sf::Joystick::PovY:
        if (strength < 0)
            result.first = aPOVup;
        else
            result.first = aPOVdown;
        break;
    }
...
bool operator==(Key const & lhs, Key const & rhs)
{
    if (lhs.type_ == rhs.type_)
    {
        ...
        case Key::kJoyButton:
            return lhs.code_.joyButton_ == rhs.code_.joyButton_ &&
                   lhs.joyID_ == rhs.joyID_;
        case Key::kJoyAxis:
            return lhs.code_.joyAxis_ == rhs.code_.joyAxis_ &&
                   lhs.joyID_ == rhs.joyID_;
        }
    }
    return false;
...
std::istream & operator>>(std::istream & stream, Key & output)
{
    ...
    if (output.type_ != Key::kKeyBoard)
        stream >> output.joyID_;
    return stream;
}
