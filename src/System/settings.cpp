...
int C_EnabledWeapons = weapons::wAFK47 | weapons::wShotgun | weapons::wFlubba |
                       weapons::wFist | weapons::wRocketLauncher |
                       weapons::wROFLE | weapons::wBurner | weapons::wH2OMG;
int C_EnabledSpecials = specials::sBlast | specials::sFreeze | specials::sHeal |
                        specials::sFireWall | specials::sShocker;
...
bool save()
{
    ...
    if (outStream)
    {
        ...
        outStream << "// nearly all these options can be changed with the "
                     "in-game menu, too."
                  << std::endl
                  << std::endl;
        ...
        outStream << "[globalParticleCount] " << C_globalParticleCount
                  << std::endl;
        outStream << "[globalParticleLifeTime] " << C_globalParticleLifeTime
                  << std::endl;
        outStream << "[showFPS] " << (C_showFPS ? "true" : "false")
                  << std::endl;
        outStream << "[showParticleCount] "
                  << (C_showParticleCount ? "true" : "false") << std::endl;
        outStream << "[showLatency] " << (C_showLatency ? "true" : "false")
                  << std::endl;
        outStream << "[fullScreen] " << (C_fullScreen ? "true" : "false")
                  << std::endl;
        outStream << "[vsync] " << (C_vsync ? "true" : "false") << std::endl;
        outStream << "[drawAIPath] " << (C_drawAIPath ? "true" : "false")
                  << std::endl;
        outStream << "[iDumb] " << (C_iDumb) << std::endl;
        outStream << "[adaptiveParticleCount] "
                  << (C_adaptiveParticleCount ? "true" : "false") << std::endl;
        outStream << "[drawBotJobs] " << (C_drawBotJobs ? "true" : "false")
                  << std::endl;
        outStream << "[drawZones] " << (C_drawZones ? "true" : "false")
                  << std::endl;
        ...
        outStream << "[playerIKeys] " << C_playerIup << " " << C_playerIright
                  << " " << C_playerIleft << " " << C_playerIfire << " "
                  << C_playerISpecialKey << std::endl;
        outStream << "[playerIColor] " << C_playerIColor.r() << " "
                  << C_playerIColor.g() << " " << C_playerIColor.b()
                  << std::endl;
        outStream << "[playerITeamColor] " << C_playerITeamColor.r() << " "
                  << C_playerITeamColor.g() << " " << C_playerITeamColor.b()
                  << std::endl;
        outStream << "[playerITeamL] " << (C_playerIteamL ? "true" : "false")
                  << std::endl;
        outStream << "[playerITeamR] " << (C_playerIteamR ? "true" : "false")
                  << std::endl;
        ...
        outStream << "0" << std::endl;
        outStream << "[playerIIKeys] " << C_playerIIup << " " << C_playerIIright
                  << " " << C_playerIIleft << " " << C_playerIIfire << " "
                  << C_playerIISpecialKey << std::endl;
        outStream << "[playerIIColor] " << C_playerIIColor.r() << " "
                  << C_playerIIColor.g() << " " << C_playerIIColor.b()
                  << std::endl;
        outStream << "[playerIITeamColor] " << C_playerIITeamColor.r() << " "
                  << C_playerIITeamColor.g() << " " << C_playerIITeamColor.b()
                  << std::endl;
        outStream << "[playerIITeamL] " << (C_playerIIteamL ? "true" : "false")
                  << std::endl;
        outStream << "[playerIITeamR] " << (C_playerIIteamR ? "true" : "false")
                  << std::endl;
        ...
        outStream << "[networkTeamRed] "
                  << (C_networkPlayerI ? "true" : "false") << std::endl;
        outStream << "[showInfoHide] " << (C_showInfoHide ? "true" : "false")
                  << std::endl;
        outStream << "[showInfoSB] " << (C_showInfoSB ? "true" : "false")
                  << std::endl;
        outStream << "[showInfoDM] " << (C_showInfoDM ? "true" : "false")
                  << std::endl;
        outStream << "[showInfoTDM] " << (C_showInfoTDM ? "true" : "false")
                  << std::endl;
        outStream << "[showInfoCK] " << (C_showInfoCK ? "true" : "false")
                  << std::endl;
        outStream << "[showSelectLanguage] "
                  << (C_showSelectLanguage ? "true" : "false") << std::endl;
        outStream << "[showToolTips] " << (C_showToolTips ? "true" : "false")
                  << std::endl;
        outStream << "[languageID] " << C_languageID << std::endl;
        outStream << "[highStarResolution] " << (C_StarsHigh ? "true" : "false")
                  << std::endl;
        outStream << "[starField] " << C_StarField << std::endl;
        outStream << "[shaders] " << (C_shaders ? "true" : "false")
                  << std::endl;
        outStream << "[resolutionX] " << C_resX << std::endl;
        outStream << "[resolutionY] " << C_resY << std::endl;
        outStream << "[colorDepth] " << C_colorDepth << std::endl;
        outStream << "[audioRandom] " << (C_audioRandom ? "true" : "false")
                  << std::endl;
        ...
        outStream << "[enabledSpecials] " << C_EnabledSpecialsByUser
                  << std::endl;
    ...
    {
        std::cout << "Faild to save configuration file " << C_configPath
                  << "mars.cfg!" << std::endl;
        return false;
    }
}
...
// create it
    ...
    if (*home.rbegin() != '/')
        home += '/';
    mkdir((home + "Library/Preferences/mars/").c_str(),
          S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
#endif
    ...
    if (C_configPath == "")
    {
        ...
        if (success)
            std::cout << "Found " << C_configPath << "mars.cfg" << std::endl;
        else
            std::cout << "Found nothing. Will create a new one." << std::endl;
    }
    ...
    if (C_dataPath == "")
    {
        ...
        else if (std::ifstream("/usr/share/marsshooter/locales/English.txt"))
        {
            ...
            success = true;
                     ...
                     "/usr/share/games/marsshooter/locales/English.txt"))
        {
            C_dataPath = "/usr/share/games/marsshooter/";
            success = true;
        }
        else if (std::ifstream(
                     "/usr/local/share/games/marsshooter/locales/English.txt"))
        {
            C_dataPath = "/usr/local/share/games/marsshooter/";
            success = true;
        }
        else if (std::ifstream(
                     "/usr/local/share/marsshooter/locales/English.txt"))
        {
            C_dataPath = "/usr/local/share/marsshooter/";
            success = true;
        }
        else if (std::ifstream(
                     "/usr/local/games/marsshooter/locales/English.txt"))
        {
             ...
             it != lines.end(); ++it)
        {
            ...
            else if (inputLine == "[showFPS]")
            {
                ...
                if (value == "true")
                    ...
                    std::cout << value << " is a bad value for " << inputLine
            ...
            else if (inputLine == "[showParticleCount]")
            {
                ...
                iss >> value;
                if (value == "true")
                    C_showParticleCount = true;
                else if (value == "false")
                    C_showParticleCount = false;
                else
                    std::cout << value << " is a bad value for " << inputLine
                              << ". Use true or false instead.\n";
            }
            ...
            else if (inputLine == "[adaptiveParticleCount]")
            {
                ...
                iss >> value;
                if (value == "true")
                    C_adaptiveParticleCount = true;
                else if (value == "false")
                    C_adaptiveParticleCount = false;
                else
                    std::cout << value << " is a bad value for " << inputLine
                              << ". Use true or false instead.\n";
            }
            ...
            {
                iss >> C_playerIup >> C_playerIright >> C_playerIleft >>
                    C_playerIfire >> C_playerISpecialKey;
            }
            else if (inputLine == "[playerIIKeys]")
            {
                iss >> C_playerIIup >> C_playerIIright >> C_playerIIleft >>
            ...
            else if (inputLine == "[playerIITeamL]")
            {
                ...
                iss >> value;
                if (value == "true")
                    C_playerIIteamL = true;
                else if (value == "false")
                    C_playerIIteamL = false;
                else
                    std::cout << value << " is a bad value for " << inputLine
                              << ". Use true or false instead.\n";
            }
            else if (inputLine == "[playerIITeamR]")
            {
                ...
                iss >> value;
                if (value == "true")
                    C_playerIIteamR = true;
                else if (value == "false")
                    C_playerIIteamR = false;
                else
                    std::cout << value << " is a bad value for " << inputLine
                              << ". Use true or false instead.\n";
            }
            ...
            else if (inputLine == "[networkTeamRed]")
            {
                ...
                iss >> value;
                if (value == "true")
                    C_networkPlayerI = true;
                else if (value == "false")
                    C_networkPlayerI = false;
                else
                    std::cout << value << " is a bad value for " << inputLine
                              << ". Use true or false instead.\n";
            }
            ...
            else if (inputLine == "[showSelectLanguage]")
            {
                ...
                iss >> value;
                if (value == "true")
                    C_showSelectLanguage = true;
                else if (value == "false")
                    C_showSelectLanguage = false;
                else
                    std::cout << value << " is a bad value for " << inputLine
                              << ". Use true or false instead.\n";
            }
            ...
            else if (inputLine == "[enabledSpecials]")
            {
                ...
                std::cout << inputLine << " is a bad option in " << C_configPath
                          << "mars.cfg!\n";
        }
    }
    else
    {
        if (save())
            std::cout << "Created " << C_configPath
                      << "mars.cfg, using default settings.\n";
    }
    return true;
}
} // namespace settings
