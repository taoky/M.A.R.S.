...
enum GameType
{
    ...
    gSpaceBall,  ///< The original M.A.R.S. GameType. A little bit like soccer:
                 ///< Each team tries to destroy the opponents home planet by
                 ///< dropping a huge ball on it.
    gDeathMatch, ///< Classic Deathmatch. Smash as many opponents as possible.
                 ///< Be the first to reach the frag limit.
    gTeamDeathMatch, ///< Classic DeathMatch, but in teams: Smash as many
                     ///< opponents as possible. Be in the team which first
                     ///< reaches the frag limit.
    gCannonKeep, ///< A funny game, where both teams aim on keeping the control
                 ///< of gigantic weapon of mass destruction in order to smash
                 ///< the oponents home planet. The team which is the first to
...
/// Returns false, when there is currently a transition from one game to
/// another.
bool active();
