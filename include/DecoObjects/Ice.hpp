    ...
    void draw() const
    {
        if (object_->visible_ && object_->frozen_ > 0.f)
        {
            ...
            if (object_->frozen_ > 0)
            {
                ...
                glBegin(GL_QUADS);
                glTexCoord2f(posX * 0.5f, posY * 0.5f);
                glVertex2f(-object_->radius_ * 1.5f, -object_->radius_ * 1.5f);
                glTexCoord2f(posX * 0.5f, (posY + 1) * 0.5f);
                glVertex2f(-object_->radius_ * 1.5f, object_->radius_ * 1.5f);
                glTexCoord2f((posX + 1) * 0.5f, (posY + 1) * 0.5f);
                glVertex2f(object_->radius_ * 1.5f, object_->radius_ * 1.5f);
                glTexCoord2f((posX + 1) * 0.5f, posY * 0.5f);
                glVertex2f(object_->radius_ * 1.5f, -object_->radius_ * 1.5f);
                glEnd();
            }
