...
/// (SPACE_X_RESOLUTION, SPACE_Y_RESOLUTION)) is transformed to a pixel location
/// on the screen, depending on the current window resolution. The given
/// location is clamped to the screen resolution, so every text will stay inside
/// the screen. This function draws the the text exactly on pixels so it won't
...
/// on the screen, depending on the current window resolution. The given
/// location is clamped to the screen resolution, so every text will stay inside
/// the screen. This function won't snap the text to pixels, therefore it can be
/// "in between" two pixels. This will look blurry, which is fine for moving
...
/// which will lead to non textured texts which are drawn after. Simply employ
/// this function when drawing text, which causes such problems. It's kinda
/// hacky and I really don't know what's going on, but, at least, it works. Any
/// improvement is highly appreciated! This function draws an invisible really
/// small text in the corner of the screen, which fixes this behaviour.
void drawFooText();
